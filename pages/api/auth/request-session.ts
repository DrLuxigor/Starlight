...
import { createSession, validateToken } from '../../../lib/auth/server/server-auth';
export default async function handler(
    ...
    if(token) {
        ...
        if(decoded) {
            ...
            if(cookie) {
                res.setHeader('Set-Cookie', "session="+ cookie +"; HttpOnly; Secure; SameSite=Strict; Path=/; Priority=High");
